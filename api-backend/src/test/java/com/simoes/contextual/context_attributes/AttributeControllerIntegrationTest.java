...
class AttributeControllerIntegrationTest {
  ...
  void Given_AuthenticatedUser_When_GetAllAttributes_Then_ReturnListOfAttributes()
          throws Exception {
    mockMvc
            .perform(get("/api/v1/users/me/attributes").contentType(MediaType.APPLICATION_JSON))
  ...
  void Given_AuthenticatedUser_When_GetContextualAttributes_Then_ReturnsFilteredVisibleAttributes()
    ...
    mockMvc
            .perform(
                    get("/api/v1/users/me/attributes/{contextId}", testContextGeneral.getId())
  ...
  Given_AuthenticatedUserAndNoVisibleAttributesForContext_When_GetContextualAttributes_Then_ReturnsEmptyList()
    ...
    mockMvc
            .perform(
                    get("/api/v1/users/me/attributes/{contextId}", emptyContext.getId())
                            .contentType(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$").isArray())
  ...
  Given_AuthenticatedUserAndExistingAttribute_When_UpdateAttribute_Then_AttributeIsUpdatedAndReturned()
    ...
    mockMvc
            .perform(
                    put("/api/v1/users/me/attributes/{attributeId}", attrEmailVisibleGeneral.getId())
                            .contentType(MediaType.APPLICATION_JSON)
  ...
  void Given_AuthenticatedUserAndNonExistentAttribute_When_UpdateAttribute_Then_ReturnsNotFound()
    ...
    mockMvc
            .perform(
                    put("/api/v1/users/me/attributes/{attributeId}", nonExistentAttribute.getId())
                            .contentType(MediaType.APPLICATION_JSON)
  ...
  Given_AuthenticatedUserAndExistingAttribute_When_DeleteAttribute_Then_AttributeIsDeletedAndReturnsNoContent()
          throws Exception {
    mockMvc
            .perform(delete("/api/v1/users/me/attributes/{attributeId}", attrEmailVisibleGeneral.getId()))
  ...
  void Given_AuthenticatedUserAndNonExistentAttribute_When_DeleteAttribute_Then_ReturnsNotFound()
          throws Exception {
    mockMvc
            .perform(delete("/api/v1/users/me/attributes/{attributeId}", "non-existent-attr-delete"))
            .andExpect(status().isNotFound());
  ...
  @Test
  @DisplayName("Should create and update attributes in bulk successfully")
  @WithMockUser(username = "attribute.user.int")
  void Given_AuthenticatedUserAndAttributeList_When_SaveAttributes_Then_AttributesAreCreatedAndUpdated()
          throws Exception {
    // 1. An update to an existing attribute
    IdentityAttribute updatedEmail = new IdentityAttribute(
            attrEmailVisibleGeneral.getId(),
            null,
            "email",
